...
// function getChartPath(activeTab) {
//   if (activeTab === "KOSPI") return "/chart/vkospi.png";
//   return "/chart/vkosdaq.png";
// }
...
//   const GAP_AFTER_FILTER = isMobile ? 24 : 32;
//   const GAP_BEFORE_CHART = isMobile ? 14 : 20;
...
//   const currentLimit = useMemo(() => PERIOD_TO_LIMIT[period] || 200, [period]);
//   const currentPath = useMemo(() => getChartPath(activeTab), [activeTab]);

//   // 차트 이미지 fetch + polling
//   useEffect(() => {
//     let mounted = true;
//     let timeoutId = null;
//     let controller = null;
...
//         setChartSrc(objectUrl);
//       } catch (e) {
//         if (!mounted) return;
//         if (e?.name === "CanceledError") return;

//         console.error("차트 이미지 호출 실패:", e);
//         if (e?.response?.status === 401) {
//           setError("인증 오류(토큰 만료/재발급 실패)");
//         } else if (e?.response?.status === 404) {
//           setError("차트 API 경로를 찾을 수 없습니다. (/chart/*.png 확인)");
//         } else {
//           setError("차트 이미지 API 호출 실패");
//         }
//       } finally {
...
//       if (mounted) {
//         timeoutId = setTimeout(fetchChart, POLL_MS);
//       }
//     };
...
//           }}
//         >
//           VOLATILITY DASHBOARD (IMAGE MODE)
//         </h2>
...
========================= */
let accessToken = null;
let accessTokenExpMs = 0; // epoch ms
let issuePromise = null;
let refreshPromise = null;
...
function getJwtExpMs(token) {
  try {
    ...
    if (!Number.isFinite(expSec)) return 0;
    return expSec * 1000;
  } catch {
    return 0;
  }
...
function setAccessToken(token) {
  accessToken = token || null;
  accessTokenExpMs = token ? getJwtExpMs(token) : 0;
}

function hasUsableToken(skewMs = 45_000) {
  // 현재 시각 + 45초가 exp보다 작아야 사용 가능
  return !!accessToken && Date.now() + skewMs < accessTokenExpMs;
}
...
async function issueAccessToken() {
  ...
  const res = await api.post(
    ...
    {},
    {
      headers: { "X-Client-Key": AUTH_CLIENT_KEY },
      skipAuth: true,
    },
  );
 ...
 * forceRefresh=false:
 *   - 유효 토큰 있으면 그대로 사용
 *   - 없거나 임박 만료면 refresh -> 실패 시 issue
 *
 * forceRefresh=true:
 *   - 무조건 refresh 시도 -> 실패 시 issue
 */
...
// 응답 인터셉터: 401이면 refresh(강제) 후 1회 재시도
api.interceptors.response.use(
  (res) => res,
  async (error) => {
    const original = error?.config || {};
...
function getChartPath(activeTab) {
  return activeTab === "KOSPI" ? "/chart/vkospi.png" : "/chart/vkosdaq.png";
}
...
function Charts() {
  ...
  useEffect(() => {
    ...
    const fetchChart = async () => {
      // 기존 이미지 없을 때만 스피너 강하게 노출
      if (!chartSrc) setLoading(true);
      setError(null);
  ...
  }, [currentPath, currentLimit]); // chartSrc를 의존성에 넣지 않음(무한 루프 방지)
